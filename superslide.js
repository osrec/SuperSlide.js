// Released into the wild by Bx & OSREC Financial
// Check out Bx for free business management tools @ https://usebx.com
// Check out OSREC Financial @ https://osrec.co.uk
...
OSREC.superslide = function(p)
{
		...
		case 'slideLeft':
		{
			...
			me.p.touchStartFunction = function(e)
			{
				...
				var touchMoveFunction = function(e)
				{
					...
					if(translation > me.p.slider.offsetWidth) { translation = me.p.slider.offsetWidth; }
					if(translation < 0) { translation = 0; }

					me.p.slider.style.transform = `translate3d(${translation}px, 0, 0)`;
					if(me.p.slideContent) { me.p.content.style.transform = me.p.slider.style.transform; }
				...
				var touchEndFunction = function(e)
				{
					...
					me.p.slider.style.transition = `${me.p.duration}s`;
					if(me.p.slideContent) { me.p.content.style.transition = `${me.p.duration}s`; }

					if(!me.p.isOpen && delta >= me.p.openThreshold) { me.open(true); }
					else if(me.p.isOpen && delta < -me.p.closeThreshold) { me.close(true); }
					else { me.p.isOpen ? me.open(true) : me.close(true); } // Return to existing state if not gone beyond threshold
	...
	{
		me.p.contentTouchStartFunction = function(e) { e.preventDefault(); };
	}
...
OSREC.superslide.prototype.open = function(doPartialAnimation)
{
	...
	if(!doPartialAnimation && me.p.isOpen) { return Promise.resolve(); }
	...
	var promise = new Promise(function(resolve, reject)
	{
		...
		if(me.p.slideContent) { me.p.content.style.transform = me.p.contentTransform; }
	});
...
OSREC.superslide.prototype.close = function(doPartialAnimation)
{
	...
	var promise = new Promise(function(resolve, reject)
	{
		...
		me.p.slider.style.opacity = 1;
		if(me.p.slideContent) { me.p.content.style.transform = me.p.contentTransformClose; }
	});
...
{
	// AMD Module
	define([], function() { return OSREC.superslide; });
}
else if (hasExports)
{
	// Node.js Module
